...
describe('GET /cafes', () => {
  describe('When the request is valid', () => {
    ...
    it('returns a list of cafes', () => {
      async () => {
        const response = await request(server).get('/cafes').send();

        expect(Array.isArray(response.body)).length.toBeGreaterThan(0);
      }
    });
  });
});
...
describe('POST /cafes', () => {
  describe('When the request is valid', () => {
    it('returns 201 OK', () => {
      async () => {
        const size = await request(server).get('/cafes').length;
        const response = await request(server).post('/cafes').set('Authorization', 'token').send(
          {id: 5, nombre: 'Chocolatado'}
        );
